...
routes.push({
    ...
    handler: {
        proxy: {
            uri: 'http://localhost:3453/image/location',
            passThrough: true,
            acceptEncoding: false,
            onResponse: handler.createLocationAfterImageUpload
        }
    },
    config: {
...
routes.push({
    ...
    handler: handler.getBubbleScreen,
    config: {
        ...
        validate: {
            query: validation.nearbyQueryOptional
        }
    }
});
...
routes.push({
    method: 'POST',
    path: '/locations/{locationId}/favor/toggle',
...
routes.push({
    ...
    handler: handler.postFavorLocation,
    config: {
        description: 'Favor a location by the currently logged in user',
        notes: 'returns object with new fav-count, added and removed info (bool)',
        tags: ['api', 'location', 'favor', 'favorites'],
        validate: {
            params: validation.locationId
...
routes.push({
    method: 'POST',
    path: '/locations/{locationId}/unfavor',
    handler: handler.postUnfavorLocation,
    config: {
        description: 'Unfavor a location by the currently logged in user',
        notes: 'returns object with new fav-count, added and removed info (bool)',
        tags: ['api', 'location', 'favor', 'favorites'],
        validate: {
            params: validation.locationId
