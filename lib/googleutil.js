...
fns.findNameOfPosition2 = (long, lat) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        Wreck.get(mapURL, {json: true}, (err, res, payload) => {
            ...
            //In case there are no specific place details
            else if (place.hasOwnProperty('county')) {
                cityname = place.county;
            }

            else if (place.hasOwnProperty('state')) {
                cityname = place.state;
            }

            else if (place.hasOwnProperty('country')) {
